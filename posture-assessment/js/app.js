...
const PostureApp = (() => {
  ...
  async function init() {
    ...
    showToast("正在加载姿态检测模型...", "info");
    try {
      await PoseDetector.init({ delegate: "GPU" });
      isPoseReady = true;
      showToast("模型加载完成，点击"开始分析"启动", "success");
    } catch (err) {
      console.error("[App] PoseDetector init failed:", err);
      // Try CPU fallback
      try {
        showToast("GPU加载失败，尝试CPU模式...", "warning");
        await PoseDetector.init({ delegate: "CPU" });
        isPoseReady = true;
        showToast("模型加载完成(CPU)，点击"开始分析"启动", "success");
      } catch (err2) {
        showToast("模型加载失败: " + err2.message, "error");
